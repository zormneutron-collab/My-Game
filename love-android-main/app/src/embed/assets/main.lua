...
function resetGame()
    ...
    -- أكواب الإعلانات مؤقتة: تُسحب عند بداية كل جولة جديدة
    for _, cd in ipairs(cupDefs) do
        if cd.adUnlock then
            unlockedCups[cd.id] = false
        end
    end
    -- إذا كان الكوب المختار إعلانياً، ارجع للماء
    local selectedIsAd = false
    for _, cd in ipairs(cupDefs) do
        if cd.id == selectedCup and cd.adUnlock then selectedIsAd = true end
    end
    if selectedIsAd then selectedCup = "water" end
    checkCupUnlocks()
    updatePlayerCup()
    playMusic("play")
end
