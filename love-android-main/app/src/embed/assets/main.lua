...
local function triggerDeath()
    saveScore()
    checkCupUnlocks()
    -- تأكد من حفظ التحديات عند الموت
        ...
        if o.y > SH then
            if not o.isFading then
                ...
                elseif o.type == "stone" then
                    -- الحجر يختفي بدون عقوبة = الاعب تفاداه
                    challengeTrack.rockDodged = challengeTrack.rockDodged + 1
                elseif o.type == "coffee" then
                    -- القهوة سقطت دون التقاط = الاعب تجنبها
                    challengeTrack.coffeeAvoided = challengeTrack.coffeeAvoided + 1
                    updateChallengeProgress()
                end
            end
...
function processScore(o)
    ...
    if o.type == "coffee" then
        if activeEffects.noCoffee > 0 then
            ...
            -- التقاط القهوة يكسر تسلسل التجنب
