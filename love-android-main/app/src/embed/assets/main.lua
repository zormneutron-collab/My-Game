...
local function updateDeath(dt)
    if pollAdReward() then
        if adForSkin and _skinAdUnlockCup then
            -- فتح كوب بإعلان
...
function drawSettings()
    ...
    love.graphics.setColor(1, 1, 1, 1)
    -- أيقونة + نص
    local sfxIcon = settings.soundOn and "🔊 " or "🔇 "
    love.graphics.printf(sfxIcon .. (settings.soundOn and "SFX: ON" or "SFX: OFF"),
    ...
    love.graphics.setColor(1, 1, 1, 1)
    local musIcon = settings.musicOn and "🎵 " or "🎵 "
    love.graphics.printf(musIcon .. (settings.musicOn and "Music: ON" or "Music: OFF"),
...
function drawSkinMenu()
    ...
    love.graphics.printf("-- Ad Cups (Watch Ad to Unlock) --", panelX, drawY, panelW, "center")
    drawY = drawY + 28
    col = 0
