...
local function dlog(msg)
    ...
    if #debugLog > 12 then table.remove(debugLog) end
end
...
local function writeAdCommand(cmd)
    ...
    if f then
        ...
        dlog("❌ فشل كتابة الأمر! المسار محظور")
    end
end
...
local function readAdResult()
    ...
    dlog("📩 نتيجة Java: " .. tostring(result))
    return result
end
...
function love.draw()
    ...
    love.graphics.rectangle("fill", 0, SH - 290, SW, 290)
    love.graphics.setFont(font_small)
    local dy = SH - 280
    love.graphics.setColor(1, 1, 0, 1)
    love.graphics.print("=== DEBUG ===", 10, dy); dy = dy + 22
    love.graphics.setColor(0.5, 1, 0.5, 1)
    love.graphics.print("OS: " .. love.system.getOS(), 10, dy); dy = dy + 20
    love.graphics.print("waiting: " .. tostring(adWaitingForResult), 10, dy); dy = dy + 20
    love.graphics.print("notAvail: " .. tostring(adNotAvailable), 10, dy); dy = dy + 20
    love.graphics.setColor(1, 1, 1, 1)
    for _, msg in ipairs(debugLog) do
        if dy > SH - 10 then break end
        love.graphics.print(msg, 10, dy); dy = dy + 18
    end
end
...
function love.update(dt)
    if     gameState == "play"  then updateGame(dt)
