...
local function getActivity()
    if _activity == nil and love.system.getOS() == "Android" then
        pcall(function()
            local sdl = luajava.bindClass("org.libsdl.app.SDLActivity")
            _activity = sdl:getCurrentActivity()
        end)
        if _activity == nil then
            pcall(function()
                local sdl = luajava.bindClass("org.libsdl.app.SDLActivity")
                _activity = sdl.mSingleton
            end)
        end
    end
    return _activity
